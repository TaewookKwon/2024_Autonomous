...
class latticePlanner:
    def __init__(self):
        ...
        rospy.Subscriber("vehicles_within_range", Int32, self.vehicles_within_range_callback)
        rospy.Subscriber("risk_score", Float32, self.risk_score_callback)
        
        self.min_collision_time = 999
        self.vehicles_within_range = 0
        self.risk_score = 0

        self.lattice_path_pub = rospy.Publisher('/lattice_path', Path, queue_size=1)
        self.lattice_marker_pub = rospy.Publisher('/lattice_marker', Marker, queue_size=1)  # Marker publisher 추가
        ...
        while not rospy.is_shutdown():
            if self.is_path and self.is_status and self.is_obj:
                if self.checkObject(self.local_path, self.object_data):
                    ...
                    lattice_path_index = self.calculate_cost(self.object_data, lattice_path)

                    # (7) lattice 경로 메세지 Publish
                    self.lattice_path_pub.publish(lattice_path[lattice_path_index])
                    
                    # 선택된 경로를 빨간색으로 표시
                    self.visualize_lattice(lattice_path, lattice_path_index)
                else:
    ...
    def checkObject(self, ref_path, object_data):
        is_crash = False
        for obstacle in object_data.pedestrian_list:  # obstacle -> pedestrian
    ...
    def calculate_cost(self, object_data, out_path):
        selected_lane = -1        
        lane_weight = [2, 1, 0, 1, 2]  # reference path
        
        for obstacle in object_data.pedestrian_list:
    ...
    def visualize_lattice(self, paths, selected_index):
        # 모든 경로를 시각화, 선택된 경로는 빨간색으로 표시
        # 이전 마커를 삭제
        for i in range(len(paths)):
            delete_marker = Marker()
            delete_marker.header.frame_id = "map"
            delete_marker.action = Marker.DELETE  # 이전 마커 삭제
            delete_marker.id = i
            self.lattice_marker_pub.publish(delete_marker)
        ...
        for i, path in enumerate(paths):
            ...
            # Marker orientation 초기화
            marker.pose.orientation.x = 0.0
            marker.pose.orientation.y = 0.0
            marker.pose.orientation.z = 0.0
            marker.pose.orientation.w = 1.0  # 단위 quaternion으로 설정

            # 경로의 모든 점을 marker에 추가
            ...
            marker.id = i  # 각 경로에 고유 ID 부여
            self.lattice_marker_pub.publish(marker)  # Marker 메시지 발행


    def min_collision_time_callback(self, msg):
        self.min_collision_time = msg.data

    def vehicles_within_range_callback(self, msg):
        self.vehicles_within_range = msg.data

    def risk_score_callback(self, msg):
        self.risk_score = msg.data
    ...
    def latticePlanner(self, ref_path, vehicle_status):
        ...
        vehicle_velocity = vehicle_status.velocity.x * 3.6  # [km/h]
        ...
        if len(ref_path.poses) > look_distance:
            ...
            lane_off_set = [-1.0, -0.5, 0, 0.5, 1.0]
            ...
            for end_point in local_lattice_points:
                ...
                # 생성된 경로를 map 좌표계로 변환하여 lattice_path에 추가
