...
class CollisionChecker:
    def __init__(self):
        ...
        # # LSTM 모델 초기화
        model_path = '/home/taewook/LSTM/trajectory_lstm_model_4.pth'
    ...
    def ego_info_callback(self, msg):
        ...
        # 0.1초마다 데이터 저장 로직 실행
        if current_time is not None and len(self.data_buffer) == self.max_steps * 7:
            if self.last_saved_time is not None and (current_time - self.last_saved_time) >= 0.1:
                ...
                # Dynamics 예측
                ego_vehicle_copy = copy.deepcopy(self.ego_vehicle)  # 깊은 복사
                ego_trajectory_dynamics_CTRA = self.predict_motion_CTRA_dynamics(ego_vehicle_copy, self.ego_roll, time_horizon, dt)
                del ego_vehicle_copy  # 복사한 객체 삭제
                ...
                x_values = [point.x for point in ego_trajectory_CTRV.points] + [point.x for point in ego_trajectory_CTRA.points] + [point.x for point in ego_trajectory_CV.points] + [point.x for point in ego_trajectory_dynamics_CTRA.points] + [point.x for point in ego_trajectory_lstm.points]
                y_values = [point.y for point in ego_trajectory_CTRV.points] + [point.y for point in ego_trajectory_CTRA.points] + [point.y for point in ego_trajectory_CV.points] + [point.y for point in ego_trajectory_dynamics_CTRA.points] + [point.y for point in ego_trajectory_lstm.points]
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.ego_vehicle:
                ...
                if len(self.data_buffer) == self.max_steps * 7:
                    input_tensor = torch.tensor(self.data_buffer, dtype=torch.float32).view(1, 30, 7).to(self.device)
                    ego_trajectory_lstm = self.predict_motion_LSTM(input_tensor,self.data_mean, self.data_std, 0.1)
