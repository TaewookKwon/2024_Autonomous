...
class CollisionChecker:
    def __init__(self):
        ...
        model_path2 = '/home/taewook/LSTM/trajectory_lstm_model_7_nolane.pth'
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.lstm_model = self.load_lstm_model(model_path, 7)
        self.lstm_model2 = self.load_lstm_model(model_path2, 4)
            ...
            if period_time - self.last_update_time >= 0.1:
                ...
                if hasattr(self, 'data_buffer2') :  # Check if data_buffer is defined and not empty
                    ...
                    input_data2 = [self.ego_vehicle.position[0], self.ego_vehicle.position[1],
                                  np.sqrt(self.ego_vehicle.velocity[0]**2 + self.ego_vehicle.velocity[1]**2),
                                  self.ego_vehicle.yaw
                                ]
                    ## Ego에서 추가된 부분
                    offset = [self.ego_vehicle.position[0], self.ego_vehicle.position[1], 0, 0]
                    input_data_normalize2 = [(input_data2[i]-self.data_mean[i]-offset[i]) / self.data_std[i] for i in range(4)]
                    ...
                    # 데이터가 max_steps를 초과하면 가장 오래된 데이터 삭제
                    if len(self.data_buffer2) > self.max_steps * 4:
                        self.data_buffer2 = self.data_buffer2[-self.max_steps * 4:]  # 마지막 210개 데이터 유지    

        # 현재 시간을 가져옵니다.
        current_time = msg.time.secs + msg.time.nsecs * 1e-9

        # 0.1초마다 데이터 저장 로직 실행
        if current_time is not None and len(self.data_buffer1) == self.max_steps * 7 and len(self.data_buffer2) == self.max_steps * 4:
            if self.last_saved_time is not None and (current_time - self.last_saved_time) >= 0.1:
                ...
                # LSTM2
                ego_vehicle_copy = copy.deepcopy(self.ego_vehicle)  # 깊은 복사
                input_tensor2 = torch.tensor(self.data_buffer2, dtype=torch.float32).view(1, 30, 4).to(self.device)
                if np.sqrt(self.ego_vehicle.velocity[0]**2 + self.ego_vehicle.velocity[1]**2) > 0.05: # 속도가 존재할 때,
                    ego_trajectory_lstm2 = self.predict_motion_LSTM2(input_tensor2,self.data_mean, self.data_std, dt)
                else:
