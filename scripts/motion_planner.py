...
class motionPlanner:
    def __init__(self):
        ...
        # self.lattice_path_pub = rospy.Publisher('/local_path', Path, queue_size = 1)
    ...
    #             if dis < 3: # 장애물의 좌표값이 지역 경로 상의 좌표값과의 직선거리가 2.35 미만일때 충돌이라 판단.
    #                 is_crash = True
    #                 break
    ...
    #     # is_crash 상태를 퍼블리시
    #     self.is_crash_pub.publish(is_crash)
    ...
    #     out_path = [] # out_path 변수는 생성된 경로들을 저장하는 리스트
    #     vehicle_pose_x = vehicle_status.position.x # 현재 차량의 위치를 가져옵니다.
    #     vehicle_pose_y = vehicle_status.position.y
    #     vehicle_velocity = vehicle_status.velocity.x * 3.6 # 차량의 속도를 가져오고, 이를 km/h로 변환
    ...
    #     if len(ref_path.poses) > look_distance :  # 경로의 길이가 look_distance보다 클 때만 경로를 생성
    #         #TODO: (3) 좌표 변환 행렬 생성
    #         """
    #         # 좌표 변환 행렬을 만듭니다.
    ...
    #         # 경로의 시작점과 다음 점, 그리고 끝 점의 좌표를 가져옵니다.
    ...
    #         lane_off_set = [-3.0, -1.75, -1, 1, 1.75, 3.0] # 차선 오프셋 값을 정의
    #         local_lattice_points = []
            
    #         # 각 차선 오프셋에 대해 로컬 좌표계의 끝 점을 계산
    ...
    #             out_path.append(lattice_path) # 생성된 경로를 out_path에 추가
